...
void CTSM_GEN_SendEventAction::execute(CTrackingObject* pObjectForAction)
{
  ...
  if ( CTSM_GEN_Task::getInstance()->getReceiverName() == productID )
  {
  ...
  else
  {
    ...
    //m_eventMessage.dispatch(SenderName,m_eventMessage.getReceiver());
    theMessage.dispatch(SenderName,EventReceiverNames);
  }
}

CTSM_GEN_SendEventAction::CTSM_GEN_SendEventAction(const std::string& eventSender, const std::string& eventReceiver, const std::string& eventMessage, bool isPlantEvent, long delay)
