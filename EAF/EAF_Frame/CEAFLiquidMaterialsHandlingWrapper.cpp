...
bool CEAFLiquidMaterialsHandlingWrapper::doOnChangeOfLiquidAdditionData(CEventMessage &Event, bool IsIncludedInOtherUseCase)
{
  // mrut 2014-12-10
  ...
  long TreatNo          = CDataConversion::StringToLong(TreatID); //m_pModelTask->getActTreatNo(Event);
    ...
    if (m_pPD_HEAT_DATA_LIQADD->select(HeatID, TreatID, ActPlant, CSMC_DBData::unspecLong))
      ...
      // mrut 2014-12-10
      // what about handling of return codes ??
      RetValue = ( CRS::OK == pIntfEAF->hndlEvent() );

      if (!IsIncludedInOtherUseCase)
...
{
  // mrut 2014-12-10
  // At JSW-SMS 3 , the data of the liquid material are availble only at discharge, 
  // but the use case 'change of liquid material data is not triggered. So, it makes sense to include this within the 'discharge-event'.
  bool RC  = doOnChangeOfLiquidAdditionData(Event, true);     // one     event
       RC &= doOnLiquidAdditionCharged     (Event);           // another event
