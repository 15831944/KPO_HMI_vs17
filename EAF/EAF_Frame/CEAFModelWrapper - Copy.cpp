...
double CEAFModelWrapper::m_dFeedRateReductionForOxygen = 0.;
double CEAFModelWrapper::m_dOxygenFlowRange = 0.;
double CEAFModelWrapper::m_dActualO2Flow = 0.;
...
bool CEAFModelWrapper::sequence_set_plant_status(const std::string& HeatID, const std::string& TreatID, DM::iDM_Interface_var& pDM_Interface, const std::string& DataKey)
{
  ...
  try 
  {
    ...
    //doHandleEvent(); //zhpf - add according to Jesper's suggestion;  @SANKAR - not required as per Martin
  ...
  RetValue = true;
    ...
    try 
    {
      ...
      if (m_pHDE_PP_BURNER_STEP_ENTRY->selectOrdered(HeatID, TreatID, Plant, ProdPracPt,TreatmentModeNo,CSMC_DBData::unspecLong,CSMC_DBData::unspecString,CSMC_DBData::unspecString,CSMC_DBData::unspecLong) )  
      {
        ...
        double      SpecEgy   = CInv::InvalidDouble;
        ...
        for (int i = 1; i <= m_pHDE_PP_BURNER_STEP_ENTRY->getLastRow(); i++) 
        {
          ...
          SpecEgy  = m_pHDE_PP_BURNER_STEP_ENTRY->getSPECEGY(i); //getSPECEGY(i)/1000.
          ...
          if ( CRS::OK == focusOnPracPhaseSuccess )
          {
            if (m_pHDE_PP_FLAME_PROFILE->select(HeatID, TreatID, Plant, BurnerName, FlameProfileNo))
            {
              ...
              pIntfEAF->setPracBurnStepData( DeviceNo,SpecEgy,GasFlow, OxyFlow);
            }
          }
  ...
  try 
  {
    ...
    if ( (m_pHDE_PP_PROCESS_STEP_ENTRY->selectOrdered(HeatID, TreatID, Plant, ProdPracPointer, TreatmentModeNo, CSMC_DBData::unspecLong, CSMC_DBData::unspecString, CSMC_DBData::unspecLong)) && (m_pHDE_PP_PROCESS_STEP_ENTRY->getLastRow() > 0) )
    {
      ...
      for (long j = 1; j <= m_pHDE_PP_PROCESS_STEP_ENTRY->getLastRow(); j++ )
      {
        ...
        double  StepEgy           =  m_pHDE_PP_PROCESS_STEP_ENTRY->getSPECEGY (j);
        ...
        //StepEgy /= 1000.0;                                  // kWh/t ->kWh/kg  zhpf, Jesper don't need change!
        ...
        if( CurrentPracPhase != thisPracPhase )
        {
          ...
          if ( CRS::OK == focusOnPracPhaseSuccess )
          {
            ...
            { 
              ...
              // initialisation of static variables
              // mrut 2013-09-06 merge from Jindal- JSPL, but db-class not prepared yet
              //m_dOxygenFlowRange            = m_pHDE_PP_PROCESS_PARAM->getOXYGENFLOW_VALIDITYRANGE(1);
              //m_dFeedRateReductionForOxygen = m_pHDE_PP_PROCESS_PARAM->getFEEDRATE_REDUCTION_FOROXYGEN(1);

              //m_dOxygenFlowRange            = CDataConversion::SetInvalidToDefault(m_dOxygenFlowRange           ,0.);
              //m_dFeedRateReductionForOxygen = CDataConversion::SetInvalidToDefault(m_dFeedRateReductionForOxygen,0.);
         ...
         for (i = 1; i <= m_pHDE_PP_INJECTION_STEP_ENTRY->getLastRow(); ++i) 
         {
           ...
           if (SetOfInjectionPurposes.find(PurposeCode) != SetOfInjectionPurposes.end())
           {
             ...
             if ( CRS::OK == focusOnPracPhaseSuccess )
             {
               ...
               SpecEgy = m_pHDE_PP_INJECTION_STEP_ENTRY->getSPECEGY(i);
         ...
         for (i = 1; i <= m_pHDE_PP_INJECTION_STEP_ENTRY->getLastRow(); ++i) 
         {
           ...
           if (SetOfInjectionPurposes.find(PurposeCode) == SetOfInjectionPurposes.end())
           {
             ...
             if ( CRS::OK == focusOnPracPhaseSuccess )
             {
               ...
               pIntfEAF->setPracManipGasLanceStepData( LanceNo, SpecEgy, GasType, Flow);   // we use Nm³/min internally, don't we ?
             }
    ...
    try 
    {
      ...
      {    
        ...
        for (int i=1; i<=m_pHD_MAT_FEED->getLastRow(); ++i) 
        {
          ...
          double SpecEgy = 0;  // (changed from type long)                    
          ...
          {
            SpecEgy = m_pHDE_PP_PROCESS_STEP_ENTRY->getSPECEGY(1);
          }
          ...
          if ( CRS::OK == focusOnPracPhaseSuccess )
          {
            if(flag == 0)
            {
              pIntfEAF->setProcPatStepData( CProcPatCodesEAF::AbsAfterStartOfArc, // MomTyp
                                            SpecEgy, // change as per Jesper; m_pHD_MAT_FEED->getEFFY_ELEC(i),  // getEFFY_ELEC(i)/1000.,
    ...
    try 
    {
      ...
      {        
        ...
        for (int i=1; i<=m_pHDE_PP_STIRR_SPEC->getLastRow(); i++) 
        {
          ...
          if ( CRS::OK == focusOnPracPhaseSuccess )
          {
            long StirringDataStatus = pIntfEAF->setProdPracBottomStirringStepData(m_pHDE_PP_STIRR_SPEC->getSPECEGY(i),m_pHDE_PP_STIRR_SPEC->getSTIRRGASTYPE(i),m_pHDE_PP_STIRR_SPEC->getSTIRRFLOW(i));        
          }
        }
    ...
    try 
    {
      ...
      if (m_pHDE_PP_PROCESS_STEP_ENTRY->selectOrdered(HeatID, TreatID, Plant, ProdPracPointer, TreatmentModeNo, CSMC_DBData::unspecLong, CSMC_DBData::unspecString, CSMC_DBData::unspecLong))
      {
        for (int i = 1; i <= m_pHDE_PP_PROCESS_STEP_ENTRY->getLastRow(); ++i) 
        {
          ...
          if ( CRS::OK == focusOnPracPhaseSuccess )
          {
            pIntfEAF->setPracManipGasLanceStepData( 1,  
              m_pHDE_PP_PROCESS_STEP_ENTRY->getSPECEGY(i), 
              DEF_GAS::O2,
              m_pHDE_PP_PROCESS_STEP_ENTRY->getO2_FLOW(i)); // we use Nm³/min internally
          }
        } 
