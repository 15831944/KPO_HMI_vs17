...
int oracleTest()
{
	std::string sURL = "ODBC_Driver@ppms_db_master2_odbc";
	std::string sLogin = "magic_test";
...
//		int nCS = sizeof(iContents) / sizeof(int);
		...
		try
		{
			myrefConn.setAutoCommit(false);
		}
		catch(cCBS_DBExc &e) {
			std::cout << "exception test date while setting autocommit off" << e.getText() << std::endl;
		}
		...
		try
		{
			...
			try {
				cCBS_ResultSet *ptRSDate = ptStmtDate->executePtQuery(std::string("select dob, Provider from provider where provider='Brandli, Warren';"));
				ptRSDate->beforeFirst();
				while (ptRSDate->next())
				{
				...
				inDate.day = 18;
				ptRSDate->updateDate(0,inDate);
				ptRSDate->updateString(1,std::string("Abraham Lincoln III"));
				ptRSDate->insertRow();
				myrefConn.commit();
				ptRSDate->moveToInsertRow();
				inDate.month = 11;
				inDate.day = 17;
				ptRSDate->updateDate(0,inDate);
				ptRSDate->updateString(1,std::string("Isaac Newton III"));
				ptRSDate->insertRow();
				myrefConn.commit();
				ptRSDate->moveToInsertRow();
				inDate.month = 6;
				inDate.day = 19;
				ptRSDate->updateDate(0,inDate);
				ptRSDate->updateString(1,std::string("Mahatma Ghandi III"));
				ptRSDate->insertRow();
				myrefConn.commit();
			}
			catch(cCBS_DBExc &e) {
			...
			myrefConn.freeStatement(ptStmtDate);
		}
		catch(cCBS_DBExc &e) {
		...
		try
		{
			cCBS_Statement *ptStmtTestCharSize = myrefConn.createPtStatement();
			std::string testQuery("select provider, address1, comment1 from provider where provider = 'Angler, Josh'");
			cCBS_ResultSet *ptRSTestCharSize = ptStmtTestCharSize->executePtQuery(testQuery);
			ptRSTestCharSize->first();
			std::string sProvider1 = std::string(ptRSTestCharSize->getChars(0));
			std::string sAddress1 = std::string(ptRSTestCharSize->getChars(1));
			std::string sComment1 = std::string(ptRSTestCharSize->getChars(2));
			std::cout << sProvider1 << "  " << sAddress1 << "  " << sComment1 << std::endl;
			myrefConn.freeStatement(ptStmtTestCharSize);
		}
		catch(cCBS_DBExc &e) {
...
//			std::cout << "native exception info: " << c_e << std::endl;
		...
		**/
		...
		try
		{
//			cCBS_PreparedStatement *ptPrepStmtNLT = myrefConn.preparePtStatement(std::string("insert into newlibtest (id, name) values(?, ?)"));
...
//			cCBS_PreparedStatement *ptPrepStmtNLT = myrefConn.preparePtStatement(std::string("insert into newlibtest set id=?, name=?, descript=?, binid=?, contents=?"));
			int ione = 1;
//			ptPrepStmtNLT->setString(0,std::string("1"));
			ptPrepStmtNLT->setInt(0,ione);
			ptPrepStmtNLT->setString(1,std::string("Name5678901234567890123456789012"));
...
//				std::cout << ireadContents[i2] << " ";
		...
		cCBS_ResultSet *ptRS = ptStmt->executePtQuery(std::string("select referrals, Provider, Retired, address1 from provider;"));
//o		cCBS_ResultSet *ptRS = ptStmt->executePtQuery(std::string("select referrals, Provider, Retired, address1, comment1 from provider;"));
...
//o				std::cout << ireferrals << ", " << sProvider << ", " << sRetired << sAddress << std::endl;
				if (ptRS->next())
				{
					if (i==10) {
...
//						ptRS->updateString(1,std::string("Brandom, David R."));
						ptRS->updateString(1,std::string("Brandli, Warren J. K."));
						ptRS->updateInt(2,3);
//o						ptRS->updateString(4,std::string("comment"));
						ptRS->updateRow();
					}
					else if (i==5)
					{
						ptRS->moveToInsertRow();
						ptRS->updateInt(0,123);
//						ptRS->updateString(1,std::string("Sven-Louis Stuecher0123456789"));
						ptRS->updateString(1,std::string("Isaac Newton IV"));
						ptRS->updateInt(2,7);
//o						ptRS->updateString(3,std::string("adr sweet honey"));
//o						ptRS->updateString(4,std::string("sweet honey"));
						ptRS->insertRow();
					}
				}
				else
		...
		try
		{
			...
			float f109 = 109.88;
			pstmt1->setString(0, std::string("1101"));
//o			pstmt1->setFloat(0, f109);
...
**/
			...
			std::cout << "exception 3 raised, end" << std::endl;
		}
		myrefConn.commit();
